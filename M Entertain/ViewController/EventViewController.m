...
- (void)viewDidLoad
{
    ...
    eventMuArray = [[NSMutableArray alloc]initWithObjects: nil];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row % 2 == 0) {
        cellImg = [UIImage imageNamed:@"container_bg_icon_line_event.png"];
    }else{
        cellImg = [UIImage imageNamed:@"container_icon_bg_line_event.png"];
    }
    
    cell.cellImg.image = cellImg;
    
    cell.lbFeedName.text = [[eventMuArray objectAtIndex:indexPath.row]feedTitle];
    cell.lbFeedContent.text = [[eventMuArray objectAtIndex:indexPath.row]feedContent];
    
    @try {
       
        NSURL* url = [NSURL URLWithString:[[eventMuArray objectAtIndex:indexPath.row]feedAttach]];
...
{
    delegate.feedTitle = [[eventMuArray objectAtIndex:[sender tag]]feedTitle];
    delegate.feedAttach = [[eventMuArray objectAtIndex:[sender tag]]feedAttach];
    delegate.feedContent = [[eventMuArray objectAtIndex:[sender tag]]feedContent];
    delegate.feedId = [[eventMuArray objectAtIndex:[sender tag]]feedId];
    ...
    UIViewController * UIVC = [story instantiateViewControllerWithIdentifier:@"DetailMapViewController"];
...
- (void)refreshTable {
    ...
    @try
    {
        [eventMuArray removeAllObjects];
        
        NSArray *newsArr= [NSJSONSerialization JSONObjectWithData:[FetchJSON getEventFeed] options:0 error:nil];
