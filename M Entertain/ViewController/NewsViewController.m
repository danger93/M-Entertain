...
- (void)viewDidLoad
{
    ...
    newsMuArray = [[NSMutableArray alloc]initWithObjects: nil];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row % 2 == 0) {
        cellImg = [UIImage imageNamed:@"container_bg_icon_line_new.png"];
    }else{
        cellImg = [UIImage imageNamed:@"container_line_icon_news.png"];
    }
    
    cell.cellImg.image = cellImg;
    
    cell.lbFeedName.text = [[newsMuArray objectAtIndex:indexPath.row]feedTitle];
    cell.lbFeedContent.text = [[newsMuArray objectAtIndex:indexPath.row]feedContent];

    @try {
        
       
        NSURL* url = [NSURL URLWithString:[[newsMuArray objectAtIndex:indexPath.row]feedAttach]];
...
{
    delegate.feedTitle = [[newsMuArray objectAtIndex:[sender tag]]feedTitle];
    delegate.feedAttach = [[newsMuArray objectAtIndex:[sender tag]]feedAttach];
    delegate.feedContent = [[newsMuArray objectAtIndex:[sender tag]]feedContent];
...
- (void)refreshTable {
    ...
    @try
    {
        [newsMuArray removeAllObjects];
        NSArray *newsArr= [NSJSONSerialization JSONObjectWithData:[FetchJSON getNewsFeed] options:0 error:nil];
